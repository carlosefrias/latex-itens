    ...
    \begin{animateinline}[poster=31, controls={play,step,stop}]{24}
        \multiframe{361}{rtheta=0+1}
        {
            \begin{tikzpicture}[scale=1.2]
                ...
                \draw[thick, rotate around={\rtheta:(-2,0)}] (-2,0) -- (-1,0);
                \draw[->, >=stealth] (-1.6,0) arc [start angle=0, end angle=\rtheta, radius=0.4];
                \draw[ultra thick, blue] (-2, 0) -- (-2, {fSeno(\rtheta)});
                \draw[thin, blue, dashed] (-2, {fSeno(\rtheta)}) -- ({degreeToRad(\rtheta)}, {fSeno(\rtheta)}) -- ({degreeToRad(\rtheta)},0);
                \draw[fill=black] ({degreeToRad(\rtheta)}, {fSeno(\rtheta)}) circle (1pt);
            \end{tikzpicture}
        }
    \end{animateinline}
\end{document}